...
var apl = {
	...
	cs: {
		...
		datsFisGens: {
			...
			nomFi: 'Selecci�n de filtros de consulta'
		},
		mscs: {
			mcsPpl: {
...
$(function() {
	...
	//Clasificaci�n dats para todos los elementos hijo de un objeto
	//mandar todos los afee a ge < generarEventos
	//Si no hay ninguna capa desde la carga inicial dl visor que elimine el bot�n mapa de la navegaci�n
    ...
    var rl = function registrarLog(t, tex, desc, complemento) {
		...
		//Extraer el csgs, parte que fall� por completo 
		if (t === 'errF-c') {
			t = 'err';
			//Tomar una acci�n fall� una consulta
		} else if (t === 'errF-csgs') {
			t = 'err';
			//Tomar una acci�n, no hay ninguna capa cargada
		} 
		if (vd(complemento)) {
    ...
    var rp = function realizarPeticion(descPeticion, url, paramsPeticion, funExito, funErr) {
        ...
        try {
			...
			rl('err', concat('Realizando la solicitud >', descPeticion, '<'), 'Sucedi� un error posterior a la recepci�n de la resp exitosa de una solicitud', {args: [url, paramsPeticion, funExito, funErr], men: err.message});
			lf(funErr);
		}
    };
	...
	var rp_pru = function realizarPeticion_pru(descPeticion, url, paramsPeticion, funExito, funErr) {
        ...
        $.ajax({
            ...
            success: function (dats) {
                ...
                var texLog = concat('Ah ocurrido un error al realizar la petici�n >', descPeticion, '<'), descLog = null;
                if (resp.status == 0) {
                    descLog = 'Al parecer el equipo est� desconectado de la red';
                } else if (resp.status == 404) {
                    descLog = 'La URl especificada no ha dado resp';
                } else if (resp.status == 500) {
                    descLog = 'Error interno del servidor';
                } else if (texErr == 'parsererror') {
                    descLog = 'Error en la conversi�n de los datos';
                } else if (texErr == 'timeout') {
                    descLog = 'La petici�n excedi� el tiempo m�ximo de espera permitido';
                } else { 
	...
	var acggr = function anexarCapaGeograficaAGrupo(alM, datsCg) {
		var alCg = datsCg.al, objGrcsgs = apl.ast.dats.atMaps[alM].acsgs[datsCg.alGrcsgs], objAtrscg = null, contador = 0,
			nombresAtributos = ['Configuraci�n', 'Informaci�n', 'Leyenda'], indNombreAtributo = 0, bAtrInformacion = false, leyenda = null;
		try {
			...
			objGrcsgs.nscsgs[alCg].eleCa.append(
				ce('img', {alt: 'Mostrar/ocultar capa geogr�fica', src: datsCg.es.vi ? 'images/layerTreeGeographic/layer-hide.png' : 'images/layerTreeGeographic/layer-show.png'}, 'c1 cup dib generated-element-node-geographic-layer-header-button', {})
			);
			$.each(objAtrscg,
				function (id, val) {
					if (objAtrscg.hasOwnProperty(id) && esOp(val) && id.indexOf('al') === -1 && id.indexOf('ele') === -1) {
						val.eleCa.append(ce('img', {alt: 'Mostrar/ocultar �rea', src: 'images/layerTreeGeographic/attribute-show.png', 'data-alatrcg': id}, 'cup di generated-element-attribute-geographic-layer-toggle-button', {}));
			...
			if (vd(datsCg.inf.anio)) {
				objAtrscg.inf.eleCu.append(ce('p', {}, 'fwb generated-element-attribute-geographic-layer-subtitle', {}).html('A�o:'));
			...
			if (vd(datsCg.inf.desc)) {
				objAtrscg.inf.eleCu.append(ce('p', {}, 'fwb generated-element-attribute-geographic-layer-subtitle', {}).html('Descripci�n:'));
			...
			//M�todo para crear los ranges con su respectivo t�tulo
			...
			if (evoa(datsCg, 'cg', ['es', 'caLe'])) {
				...
				leyenda.onerror = function () {
					...
					rl('err', concat('Cargando la leyenda gr�fica de la capa geogr�fica de nombre ', datsCg.inf.nom, 'y de alias >', alCg, '<'), 'Ocurri� un error durante la carga de la leyenda gr�fica de una capa geogr�fica', leyenda.src);
					//</Verificar>
					ge(['acg'], objAtrscg);
				};
			} else {
			...
			rl('err', concat('Anexando capa geogr�fica >', alCg, '< al grupo del �rbol de capas del mapa >', alM, '<'), 'Mientras se anexaba un capa a un grupo de el �rbol de capas de un mapa sucedi� una excepci�n', err.message);
		}
	};
	...
	var acgm = function agregarCapasGeograficasAMapa(alM, datsCsgs) {
		...
		try {
			for (id in datsCsgs) {
				if (datsCsgs.hasOwnProperty(id)) {
					...
					if (vd(servicio)) {
						...
						//var num1 = 0, num2 = 0;
						//Pasar estos eventos a la funci�n var = ge
						...
						capaOl.getSource().on(concat(raizEvento, 'loaderror'), function(event) {
							...
							//Se est� cometiendo un error en esta solicitud
							//Que solo remueva la capa 1 vez 
							mapa.ol.removeLayer(apl.dats.csgs.dats[alCg].ol);
							//</Ojo con este alM>
							rl('err', concat('Capa: >', alCg, '<'), 'Error del servicio, puede deberse a malas especificaciones de los par�metros de configuraci�n o puede tratarse de problemas con el acceso al servidor de mapas.');
							if (mapa.ol.getLayers().getArray().length === 0) {
								rl('errF-csgs', 'No fue posible cargar ninguna capa sobre el visor geogr�fico', 'Puede deberse a errores de comunicaci�n, verifique su conexi�n a Internet');
							}
						...
						rl('err', concat('Servicio: >', objCg.alSg, '<'), 'Algo sali� mal al acceder al objeto de un servicio, puede deberse a que el alias del servicio est� mal especificado');
						objCg.e = false;
					}
				}
			}
			...
			rl('err', concat('Agregando capa geogr�fica >', id, '< al mapa >', alM, '<'), 'Mientras se generaba la tabla de contenido ocurri� una excepci�n', err.message);
		}
	};
	...
	var agacg = function anexarGrupoAlArbolDeCapasGeograficas(alM, datsAcsgs) {
		...
		try {
			...
			rl('err', 'Generando los elementos del �rbol de capas geogr�ficas', 'Mientras se generaba el �rbol de capas geogr�ficas ocurri� una excepci�n', err.message);
	...
	var cnmcp = function configurarNivelDeMenuDeConsultasPrincipal(indNiv, idOp, tex, eleB) {
		...
		if (objMcsPpl.nivs.length > indNiv) {
			for (ind = indNiv; ind < objMcsPpl.nivs.length; ind = ind + 1) {
				...
				//<OJO Modificar este borrado por algo m�s practico, incluso eliminarlo>
	...
	var ecsgsvr = function establecerCapasGeograficasVisiblesPorResolucion(alM, alCg) {
		...
		function ecgvr(cg) {
			...
			//Estudiar optimizaci�n especialmente de if (!cg.es.vi) { ya que es casi igual que en el evento
		...
		try {
			...
			switch (apl.e) {
				case 1:
					...
					break;
				case 2:
					rp_pru('Solicitud de �rbol de consultas', 'http://10.10.20.42:8080/SIUPRAWS/webresources/obj.departamentos', {}, [main, e], [main, 'errF']);
					break;
				case 3:
											...
											function () {
												eleB.mouseup(
													function () {
														ovm(true, 
															function () {
																//<OJO estos estilos se deben aplicar bien, verificar si es buena idea que el que llame la funci�n lanzarVentanaModal seleccione los estilo de este>
			...
			rl('err', 'Cargando datos a la aplicaci�n', 'Durante la carga de alg�n dato se cometi� una excepci�n y es posible que no se pueda continuar', {dats: arguments, men: err.message})
		}
    };
	...
	var racsm = function removerArbolDeCapasGeograficasDeMapa(alM) {
		...
		objM.acsgs.e = false;
		rl('adv', concat('No se ha cargado ning�n elemento en el �rbol de capas del mapa >', alM, '<'), 'Ninguna capa geogr�fica fue representada en el �rbol de capas, al parecer por problemas de conexi�n con los servicios');
	};
	...
	var rcgac = function removerCapaGeograficaDeElementoDeArbolDeCapas(alM, alGrcsgs, alCg) {
		...
		if (objGrcsgs.eleA.find('ul').children().length === 0) {
			...
			rl('adv', concat('Se ha eliminado el elemento >', alGrcsgs, '< del �rbol de capas del mapa >', alM, '<'), 'Ninguna capa geogr�fica de este grupo pudo ser representada en el �rbol de capas, al parecer por problemas de conexi�n con el servicio');
					...
					$.each(ele,
					   function (ind, val) {
							if (esF(val.addEventListener)) {
								...
								rl('adv', concat('Agregando funci�n al evento del elemento >', ele.selector, '<'), 'No se ha sido posible agregar una funci�n al evento de un elemento dado que no se ha podido establecer la manera de hacerlo');
							}
					   }
					);
				}
    ...
    var ge = function generarEventos(ts, dats) {
		...
		//Eventos de: acg: Atributos de capa geogr�fica, dih: data- interacci�n hover,  gcg: grupo de capas geogr�ficas, m: mapa, mc: men� de consultas, n: navegaci�n, ncg: nodo capa geogr�fica, omc: opci�n de men� de consultas, vm: ventana modal
		...
		try {
			...
			if (ts.indexOf('mc') > -1) {
				//Estos m�todos solo se pueden llamar cuando se hay�n cargado todos los men�s
				afee($('[data-i="ratonsobreEleBMcs"]'), 'mouseenter', 
					function (arg) {
						var ele = $(arg.target), elePa = $(ele.parent());
						//Ocultar todos los dem�s men�s
				...
				//Cargar consultas r�pidas tambi�n
			}
			if (ts.indexOf('n') > -1) {
			...
			if (ts.indexOf('ncg') > -1) {
				...
				$.each(dats.eleCu.find('.generated-element-attribute-geographic-layer-range'),
					function(ind, val) {
						//Me�todo general para setar lo valores tanto en el mapa como en la variable de cg.conf
						var ele = $(val);
						afee(ele, 'change',
							function (arg) {
			...
			if (ts.indexOf('omc') > -1) {
				//Analizar si aplica para todos los men�s de consutla o ajustarlo para tal comportamiento
			...
			rl('err', concat('Generando eventos de >', String(ts), '<'), 'Se produjo una excepci�n al generar eventos de controles', {dats: {d0: ts, d1: dats}, men: err.message});
		}
    };
		...
		if (esN(paso)) {
            switch (paso) {
                case 0:
                    ...
                    break;
                case 1:
					/*<DATOS FICTISIOS, Ojo con los �rdenes>*/
					dats = {
						acsgs: {
							grcsgsCt:{
								inf:{ 
									desc: 'Cattograf�a tem�tica del pa�s', 
									nom:'Cartograf�a tem�tica', 
									palsCla:'cartograf�a, tem�tica'
								}
							},
							grcsgsZe:{inf:{desc:'Zonas de exclusi�n legal 2010, 2014', nom:'Zonas de exclusi�n', palsCla:'zonas, exclusi�n, 2010, 2014'}},
							grcsgsWu:{inf:{desc:'WMS UPRA', nom:'WMS UPRA', palsCla:'WMS, UPRA'}}
						},
						ssgs: {
							sgCt: {acc:0, conf: {tServSg:'Geoserver', url:'http://10.10.30.8:8082/geoserver/carto_tematica/wms?'}, es:{impor: false}, inf:{desc:'Primer servicio de prueba', nom:'UPRA cartograf�a tem�tica', palsCla:'UPRA, cartograf�a, tem�tica'}},
							sgTest: {conf: {tServSg:'Geoserver', url:'http://10.10.30.8:8082/geoserver/test/wms?'}, es:{impor: false}, inf:{desc:'Segundo servicio de prueba', nom:'UPRA test', palsCla:'UPRA, test, zonificac�n'}},
						...
						csgs: {
							...
							,cg1: {acc: 0, alGrcsgs: 'grcsgsCt', alSg: 'sgCt', conf: {reMa: 6.866455078125E-4, reMi: 0.0054931640625, fis: null, lims: '-85.023, -7.228, -66.847, 15.167', nomCg: 'icare', opa: 1, tCg: 'WMS.TILE'}, es: {autoIden: false, viVisGen: true, caLe: true, iden: true, or: true, transp: true, vi: true}, inf: {anio: 2014, desc: '�ndice municipal de formalidad estimado a partir de ICARE', esc: '1:100.000', fu: 'UPRA', img: 'images/layersGeographic/ic.png', nom: 'ICARE', palsCla: '�ndice, formalidad, ICARE'}}
							,cg2: {acc: 0, alGrcsgs: 'grcsgsCt', alSg: 'sgCt', conf: {reMa: 6.866455078125E-4, reMi: 0.0054931640625, fis: null, lims: '-85.001, -7.202, -66.803, 15.104', nomCg: 'produccion_nal_mpio', opa: 0.5, tCg: 'WMS.TILE'}, inf: {anio: 2014, desc: 'Producci�n nacional por municipio de Colombia', esc: '1:100.000', fu: 'UPRA', img: 'images/layersGeographic/pnm.png', nom: 'Producci�n nacional por municipio', palsCla: 'producci�n, nacional, municipial'}}
							//,cg3: {alGrcsgs:, alSg: 'sgCt', conf: {fis: null, nomCg: 'prod_mpio_arroz', opa: 1, sire: 'EPGS:4326'}, es: {autoIden: false, viVisGen: false, caLe: true}, inf: {anio: 2014, desc: 'Producci�n nacional por municipio de arroz en Colombia', esc: '1:100.000', fu: 'MADR', nom: 'Producci�n nacional por municipio - Arroz', palsCla: 'arroz, producci�n, nacional'}}
							...
							,cg5: {alGrcsgs: 'grcsgsWu', alSg: 'sgTest', conf: {reMa: 6.866455078125E-4, fis: null, nomCg: 'zonasexclusionlegal2014', opa: 1, sire: 'EPGS:4326', tCg: 'WMS.TILE'}, inf: {anio: 2014, fu: 'UPRA', img: 'images/layersGeographic/default.png', nom: 'Zonas de exclusi�n legal 2014'}}
						...
						mcsPpl: {
							dats: {
								...
								ti: {
									alOpPa: null,
									es: {c: true},
									tex: 'Informaci�n nacional, departamenal, municipal y mas de la "Tierra" en Colombia'
								},
								ga: {
									alOpPa: 'ag',
									nom: 'Subgaseosos',
								...
								subGa4: {
									alOpPa: 'ga',
									es: {c: true},
									tex: 'Sub Gaseoso 4 es el �ltimo sub de gaseoso'
								},
								li: {
									alOpPa: 'ag',
									es: {c: true},
					...
					try {
						...
						for (id in dats.mcsPpl.dats) {
							...
							if (indNiv !== null) {
								...
								//Generar casos en lo que esto pueda ocurrir y generar tambi�n acci�n a tomar
								console.log('Ids no encontrados al generar el men� de consultas: ', id);
							}
						}
						cnmcp(0);
						//Cargar ac� el men� de acceso r�pido
						//arrMcs = dats.mcsAr.dats;
						//Scrolls a uls de opciones
						ge(['mc']);
					} catch(err) {
						rl('err', 'Generando el men� de consultas', 'Mientras se generaba menu de consultas ocurri� una excepci�n', err.message);
				...
				case 3:
					...
					dats = {
						...
						ots: {
							dats: {
								ot2: {
									dats: ['Consolidado', 'Participaci�n', 'Sumatoria'],
					...
					if (!vd(objC.fis)) {
						...
						for (var idI in dats) {
							...
							for (var idJ in objFichaK) {
								//Ajustar para que seg�n el tipo de elemento HTML se ajusten las consultas > Tema 1
								...
								if (objFichaK[idJ] === true) {
									...
									for (var idK in objDats) {
										if (idK !== 'elesFi')  {
											...
											//Si un elemento por ejemplo periodos solo viene para un a�o entonces quitar el select de a�os y quitar el option de meses si solo viene por meses
					...
					if (!vd(objFisSels)) {
						objFisSels = {};
						lvm('Selecci�n de filtros de consulta', 1, null, objC.fis.eleCu, true,
							[
								function () {
									for(var idI in objC.fis.dats) {
										for (var idJ in objC.fis.dats[idI].dats) {
											...
											if(!vd(objFichaI.ele) || objFichaI.ele[0].checked ) {
												...
												//Establecer en la validaci�n si el tipo es select, pensar en otros tipos de dropdowns que puedan aplicar o de tipo de filtros
												//Validaci�n de datos m�nimos y de valores selccionados padres e hijos, es decir si est� seleccionado un input padre que seleccione opciones hijas
				...
				case 4:
					...
					//Cargar el mapa y poner las capas entratntes en primer �rden
						...
						ast: {
							atMaps: {
								...
								dats: {
									...
									csgs: {
										cg5: {alGrcsgs: 'csNuevas', alSg: 'sgTest', conf: {reMa: 6.866455078125E-4, fis: null, nomCg: 'zonasexclusionlegal2010', opa: 1, sire: 'EPGS:4326', tCg: 'WMS.TILE'}, inf: {anio: 2014, fu: 'UPRA', img: 'images/layersGeographic/default.png', nom: 'Zonas de exclusi�n legal 2014'}}
									}
								},
								tex: 'Super mapa'
							},
							...
							atInf: {
								//titulo principal, subtitulos, textos, galeria de im�genes
							},
							atTabs: {
								plantilla: 1,
								dats: [
									{
										...
										records: [
											{recid: 1, campo1: 'CAQUET�',   campo2: 34,   campo3: 37.9, campo4: 25},
					...
					try {
							...
							if (esA(dats.ast[idI].dats)) {
								for (ind = 0; ind < dats.ast[idI].dats.length; ind = ind + 1) {
									switch (idI) {
										...
										case 'atTabs':
											indFicha = ind + 1;
											//Funci�n para remover los templates de las grillas sin quemar su id, crear un variable # de templates en el objeto de ast
						...
						rl('err', 'Configurando los datos de una consulta', 'Sucedi� un error mientras se intentaba configurar una consulta', {dats: dats, men: err.message, err: err});
					}
					meApl(5);
					break;
				case 5:
            ...
            rl(paso, 'Fall� del sistema', 'El sistema cometi� un error grave y es posible que no se pueda continuar con la ejecuci�n del programa');
        }
